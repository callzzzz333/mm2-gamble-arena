...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("Next round for battle:", battleId);
    ...
    console.log(`Round ${nextRound} of ${battle.rounds}`);
    ...
    // Process round - open all cases for each player
    ...
    for (const participant of participants) {
      ...
      const roundValue = userResults.reduce((sum, r) => sum + r.value, 0);
      const items = Array.isArray(participant.items_won) ? participant.items_won : [];
      
      await supabase
        .from("case_battle_participants")
        .update({
          total_value: parseFloat(participant.total_value) + roundValue,
    ...
    return new Response(
      JSON.stringify({ success: true, round: nextRound, results: roundResults }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error) {
